...
// .wrangler/tmp/pages-vQAcdF/functionsWorker-0.1729981883470495.mjs
...
var PasswordValidator = class {
  ...
  static {
    __name2(this, "PasswordValidator");
  }
  /**
...
var InputValidator2 = class {
  static {
    __name(this, "InputValidator2");
  }
  static {
    __name2(this, "InputValidator");
  }
  /**
...
var AuthUtils2 = class {
  static {
    __name(this, "AuthUtils2");
  }
  static {
  ...
  static getUserAgent(request) {
    ...
    return userAgent ? InputValidator2.sanitizeInput(userAgent) : void 0;
  }
  /**
...
var RATE_LIMIT_CONFIGS2 = {
  // Authentication endpoints - more permissive for development
  LOGIN: {
    windowMs: 5 * 60 * 1e3,
    // 5 minutes (reduced window)
    maxRequests: 20,
    // 20 attempts per IP per 5 minutes (increased)
  ...
  REGISTER: {
    ...
    // 15 minutes  
    maxRequests: 20,
    // 20 registrations per IP per 15 minutes (increased)
    skipSuccessfulRequests: true,
...
var SecurityHeadersManager = class {
  ...
  static {
    __name2(this, "SecurityHeadersManager");
  }
  /**
  ...
  createMiddleware() {
    return async (request) => {
      ...
      if (!result.valid) {
        return {
          authorized: false,
          error: AuthUtils2.createErrorResponse(result.error)
        };
      }
      return {
        authorized: true,
...
var SecurityLogger2 = class {
  static {
    __name(this, "SecurityLogger2");
  }
  static {
    __name2(this, "SecurityLogger");
  }
  /**
...
var SecurityMiddleware2 = class {
  static {
    __name(this, "SecurityMiddleware2");
  }
  static {
    __name2(this, "SecurityMiddleware");
  }
  ...
  async applySecurityChecks(request, options = {}) {
    const origin = request.headers.get("Origin");
    const clientIP = AuthUtils2.getClientIP(request) || "unknown";
    ...
    try {
      ...
      if (options.allowedMethods) {
        ...
        if (!methodResult.valid) {
          const response = AuthUtils2.createErrorResponse(methodResult.error);
          return {
            allowed: false,
      ...
      if (!sizeResult.valid) {
        const response = AuthUtils2.createErrorResponse(sizeResult.error);
        return {
          allowed: false,
          response: SecurityHeadersManager.addSecurityHeaders(response)
        };
      ...
      if (options.rateLimitConfig) {
        ...
        if (!rateLimitResult.allowed) {
          SecurityLogger2.logRateLimitEvent(clientIP, endpoint, rateLimitResult.state, false);
          const response = AuthUtils2.createErrorResponse(AUTH_ERRORS.RATE_LIMITED);
      ...
      if (options.requireAuth && this.authMiddleware) {
        ...
        if (!authResult.authorized) {
          SecurityLogger2.logAuthEvent("login_failure", {
            ipAddress: clientIP,
            userAgent: AuthUtils2.getUserAgent(request),
      ...
      SecurityLogger2.logSecurityEvent("suspicious_request", {
        ipAddress: clientIP,
        endpoint,
        reason: "Security middleware error",
        severity: "high"
      });
      const response = AuthUtils2.createErrorResponse(AUTH_ERRORS.INTERNAL_ERROR);
      return {
        allowed: false,
        response: SecurityHeadersManager.addSecurityHeaders(response)
      };
...
var SecurityHelpers2 = class {
  static {
    __name(this, "SecurityHelpers2");
  }
  static {
    __name2(this, "SecurityHelpers");
  }
  /**
  ...
  static async validateRequest(request, validator) {
    ...
    if (!contentTypeResult.valid) {
      return {
        valid: false,
        response: AuthUtils2.createErrorResponse(contentTypeResult.error)
      };
    }
    ...
    if (!jsonResult.valid) {
      return {
        valid: false,
        response: AuthUtils2.createErrorResponse(jsonResult.error)
      };
    }
    ...
    if (!validationResult.isValid) {
      return {
        valid: false,
        response: AuthUtils2.createErrorResponse({
  ...
  static extractRequestMetadata(request) {
    return {
      ipAddress: AuthUtils2.getClientIP(request),
      userAgent: AuthUtils2.getUserAgent(request),
...
var PromptError2 = class extends Error {
  static {
    __name(this, "PromptError2");
  }
  ...
  static {
    __name2(this, "PromptError");
  }
};
var PromptsDatabase2 = class {
  static {
    __name(this, "PromptsDatabase2");
  }
  constructor(db) {
    this.db = db;
  }
  static {
    __name2(this, "PromptsDatabase");
  }
  /**
  ...
  validatePromptData(data, isUpdate = false) {
    if (!isUpdate) {
      ...
      if (!createData.userId || typeof createData.userId !== "string") {
        throw new PromptError2("User ID is required", "INVALID_USER_ID", 400);
      }
      if (!createData.rawRequest || typeof createData.rawRequest !== "string") {
        throw new PromptError2("Raw request is required", "INVALID_RAW_REQUEST", 400);
      }
      if (createData.rawRequest.length > 5e3) {
        throw new PromptError2("Raw request exceeds maximum length", "RAW_REQUEST_TOO_LONG", 400);
      }
      if (!createData.generatedPrompt || typeof createData.generatedPrompt !== "string") {
        throw new PromptError2("Generated prompt is required", "INVALID_GENERATED_PROMPT", 400);
      }
      if (createData.generatedPrompt.length > 1e4) {
        throw new PromptError2("Generated prompt exceeds maximum length", "GENERATED_PROMPT_TOO_LONG", 400);
      }
      if (!["MVP", "AGENTIC"].includes(createData.promptType)) {
        throw new PromptError2("Invalid prompt type", "INVALID_PROMPT_TYPE", 400);
      }
      if (!["education", "technical", "creative", "analysis", "other"].includes(createData.domain)) {
        throw new PromptError2("Invalid domain", "INVALID_DOMAIN", 400);
      }
      if (!["fr", "en"].includes(createData.language)) {
        throw new PromptError2("Invalid language", "INVALID_LANGUAGE", 400);
      }
      if (!["short", "medium", "long"].includes(createData.outputLength)) {
        throw new PromptError2("Invalid output length", "INVALID_OUTPUT_LENGTH", 400);
      }
    }
    if (data.title !== void 0) {
      if (typeof data.title !== "string" || data.title.length > 100) {
        throw new PromptError2("Title must be a string with maximum 100 characters", "INVALID_TITLE", 400);
      }
    }
    if (data.expertRole !== void 0) {
      if (typeof data.expertRole !== "string" || data.expertRole.length > 200) {
        throw new PromptError2("Expert role must be a string with maximum 200 characters", "INVALID_EXPERT_ROLE", 400);
      }
    }
    if (data.mission !== void 0) {
      if (typeof data.mission !== "string" || data.mission.length > 500) {
        throw new PromptError2("Mission must be a string with maximum 500 characters", "INVALID_MISSION", 400);
      }
    }
    if (data.constraints !== void 0) {
      if (typeof data.constraints !== "string" || data.constraints.length > 1e3) {
        throw new PromptError2("Constraints must be a string with maximum 1000 characters", "INVALID_CONSTRAINTS", 400);
      }
    }
    if (data.isFavorite !== void 0 && typeof data.isFavorite !== "boolean") {
      throw new PromptError2("isFavorite must be a boolean", "INVALID_IS_FAVORITE", 400);
    }
  }
  /**
  ...
  async createPrompt(data) {
    ...
    try {
      ...
      throw new PromptError2("Failed to create prompt", "CREATE_FAILED", 500);
    }
  }
  /**
  ...
  async getPromptById(id, userId) {
    try {
      ...
      throw new PromptError2("Failed to retrieve prompt", "GET_FAILED", 500);
    }
  }
  /**
  ...
  async getUserPrompts(userId, filters = {}, pagination = {}) {
    ...
    if (page < 1 || limit < 1 || limit > 100) {
      throw new PromptError2("Invalid pagination parameters", "INVALID_PAGINATION", 400);
    }
    ...
    try {
      ...
      throw new PromptError2("Failed to retrieve prompts", "GET_LIST_FAILED", 500);
    }
  }
  /**
  ...
  async updatePrompt(id, userId, data) {
    ...
    if (!hasOwnership) {
      throw new PromptError2("Prompt not found or access denied", "NOT_FOUND", 404);
    }
    const updateFields = [];
    const bindings = [];
    if (data.title !== void 0) {
    ...
    try {
      ...
      if (error instanceof PromptError2) {
        throw error;
      }
      throw new PromptError2("Failed to update prompt", "UPDATE_FAILED", 500);
    }
  }
  /**
   * Delete a prompt
  ...
  async deletePrompt(id, userId) {
    ...
    try {
      ...
      throw new PromptError2("Failed to delete prompt", "DELETE_FAILED", 500);
    }
  }
  /**
  ...
  async toggleFavorite(id, userId) {
    ...
    try {
      ...
      if (!currentPrompt) {
        throw new PromptError2("Prompt not found", "NOT_FOUND", 404);
      }
      ...
      throw new PromptError2("Failed to toggle favorite status", "TOGGLE_FAVORITE_FAILED", 500);
    }
  }
  /**
  ...
  async getUserPromptStats(userId) {
    try {
      ...
      throw new PromptError2("Failed to retrieve prompt statistics", "STATS_FAILED", 500);
    }
  }
};
var FavoriteValidator = class {
  ...
  static {
    __name2(this, "FavoriteValidator");
  }
  /**
...
var onRequestPost = /* @__PURE__ */ __name2(async (context) => {
  ...
  if (!env.JWT_SECRET || !env.DB) {
    SecurityLogger2.logSecurityEvent("api_error", {
      endpoint: "prompts_favorite",
      reason: "Missing environment configuration",
      severity: "critical"
    });
    return AuthUtils2.createErrorResponse({
      code: "CONFIG_ERROR",
  ...
  try {
    const db = new PromptsDatabase2(env.DB);
    ...
    const security = new SecurityMiddleware2(env.KV, env.JWT_SECRET);
    const securityResult = await security.applySecurityChecks(request, {
      rateLimitConfig: RATE_LIMIT_CONFIGS2.API,
      requireAuth: true,
      allowedMethods: ["POST"],
      endpoint: "prompts_favorite"
    });
    ...
    const clientIP = AuthUtils2.getClientIP(request);
    const userAgent = AuthUtils2.getUserAgent(request);
    let requestData = null;
    if (request.headers.get("Content-Type")?.includes("application/json")) {
      const validationResult = await SecurityHelpers2.validateRequest(
        request,
        FavoriteValidator.validateFavoriteRequest
      );
      if (!validationResult.valid) {
        SecurityLogger2.logSecurityEvent("invalid_request", {
          endpoint: "prompts_favorite",
          ipAddress: clientIP,
          userAgent,
    ...
    SecurityLogger2.logSecurityEvent("api_success", {
      endpoint: "prompts_favorite",
      ipAddress: clientIP,
      userAgent,
    ...
    return security.wrapResponse(
      SecurityHelpers2.createSecureResponse(responseData),
      request
    );
  } catch (error) {
    if (error instanceof PromptError2) {
...
async function sendResetEmail(email, token, language, env) {
  try {
    ...
    const emailData = {
      ...
      to: [email],
      subject: language === "fr" ? "R\xE9initialisation de votre mot de passe - TeachInspire" : "Reset your password - TeachInspire",
      html: emailTemplate
    };
...
function getEmailTemplate(resetUrl, language) {
  const translations = {
    fr: {
      title: "R\xE9initialisation de votre mot de passe",
      greeting: "Bonjour,",
      message: "Vous avez demand\xE9 la r\xE9initialisation de votre mot de passe pour votre compte TeachInspire.",
      button: "R\xE9initialiser mon mot de passe",
      expiry: "Ce lien expire dans 15 minutes.",
      ignore: "Si vous n'avez pas demand\xE9 cette r\xE9initialisation, vous pouvez ignorer cet email.",
      thanks: "Merci,<br>L'\xE9quipe TeachInspire"
    },
    en: {
    ...
    <body style="font-family: Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #2c3d57; background-color: #f8f7f2; margin: 0; padding: 0;">
      <div style="max-width: 600px; margin: 0 auto; background-color: white; border-radius: 8px; box-shadow: 0 4px 6px rgba(44, 61, 87, 0.1); overflow: hidden;">
        ...
        <div style="background-color: #f8f7f2; padding: 20px; text-align: center; font-size: 12px; color: #6b7280;">
          <p style="margin: 0;">TeachInspire - Votre assistant IA pour l'\xE9ducation</p>
        </div>
      </div>
    </body>
    </html>
...
__name2(getEmailTemplate, "getEmailTemplate");
var onRequestPost2 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    const emailValidation = InputValidator2.validateEmail(rawData.email);
    if (!emailValidation.isValid) {
      return AuthUtils2.createErrorResponse({
        code: "INVALID_EMAIL",
    ...
    try {
      ...
      const response = new Response(JSON.stringify({
        success: true,
        message: language === "fr" ? "Si cette adresse email existe dans notre syst\xE8me, vous recevrez un lien de r\xE9initialisation." : "If this email exists in our system, you will receive a reset link."
      }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      });
...
var onRequestPost3 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    try {
      ...
      if (!user) {
        console.log("User not found for email:", data.email);
        return AuthUtils2.createErrorResponse(AUTH_ERRORS.INVALID_CREDENTIALS);
      }
...
__name(createJWT3, "createJWT3");
__name2(createJWT3, "createJWT");
var onRequestPost7 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    const passwordValidation = InputValidator2.validateLoginRequest({
      email: emailValidation.sanitized,
      password: rawData.password
    });
    if (!passwordValidation.isValid) {
    ...
    const firstName = InputValidator2.sanitizeInput(rawData.firstName);
    const data = {
      firstName,
      email: emailValidation.sanitized,
      password: rawData.password
    ...
    console.log("Creating JWT token...");
    try {
      const token = await createJWT3({ userId, firstName: data.firstName, email: data.email }, env.JWT_SECRET);
      console.log("JWT created successfully");
...
var RegisterDatabase = class {
  ...
  async createUser(email, passwordHash, ipAddress) {
    const userId = AuthUtils2.generateSecureRandom(16);
    const stmt = this.db.prepare(`
      INSERT INTO users (
...
var onRequestPost8 = /* @__PURE__ */ __name2(async (context) => {
  ...
  if (!env.JWT_SECRET || env.JWT_SECRET.trim() === "" || !env.DB) {
    SecurityLogger2.logSecurityEvent("auth_failure", {
      endpoint: "register",
      reason: "Missing environment configuration",
      severity: "critical"
    });
  ...
  try {
    ...
    if (!securityResult.allowed) {
      SecurityLogger2.logSecurityEvent("rate_limit_exceeded", {
        endpoint: "register",
        ipAddress: AuthUtils2.getClientIP(request),
        severity: "medium"
      });
    ...
    const validationResult = await SecurityHelpers2.validateRequest(
      request,
      InputValidator2.validateRegisterRequest
    );
    ...
    if (!validationResult.valid) {
      SecurityLogger2.logAuthEvent("registration_failure", {
        ipAddress: clientIP,
        userAgent,
    ...
    if (!passwordValidation.isValid) {
      ...
      return security.wrapResponse(
        AuthUtils2.createErrorResponse({
          code: "WEAK_PASSWORD",
          message: passwordValidation.errors.join("; "),
          statusCode: 400
        }),
    ...
    if (userExists) {
      ...
      return security.wrapResponse(
        AuthUtils2.createErrorResponse(AUTH_ERRORS.USER_EXISTS),
        request
      );
    }
    ...
    SecurityLogger2.logAuthEvent("registration_success", {
      userId,
      email,
      ipAddress: clientIP,
      userAgent
    ...
    return security.wrapResponse(
      SecurityHelpers2.createSecureResponse(responseData, 201),
      // 201 Created
      request
    );
  } catch (error) {
    ...
    if (error instanceof Error) {
      if (error.message.includes("UNIQUE constraint failed")) {
        return AuthUtils2.createErrorResponse(AUTH_ERRORS.USER_EXISTS);
      }
...
async function createJWT4(payload, secret) {
  const header = { alg: "HS256", typ: "JWT" };
  const now = Math.floor(Date.now() / 1e3);
  const jwtPayload = { ...payload, iat: now, exp: now + 3600 };
  const encoder = new TextEncoder();
...
__name(createJWT4, "createJWT4");
__name2(createJWT4, "createJWT");
    ...
    try {
      ...
      const response = new Response(JSON.stringify({
        ...
        user: {
          ...
          email: resetRecord.email,
          emailVerified: true
          // Assume verified if they can reset password
        }
      }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
...
var onRequestOptions2 = /* @__PURE__ */ __name2(async (context) => {
  const { request, env } = context;
  try {
    const security = new SecurityMiddleware2(env.KV, env.JWT_SECRET);
    const securityResult = await security.applySecurityChecks(request, {
...
var PromptUpdateValidator = class {
  ...
  static {
    __name2(this, "PromptUpdateValidator");
  }
  /**
...
var onRequestOptions3 = /* @__PURE__ */ __name2(async (context) => {
  const { request, env } = context;
  try {
    const security = new SecurityMiddleware(env.KV, env.JWT_SECRET);
    const securityResult = await security.applySecurityChecks(request, {
...
var GenerateVideosResponse = class {
  ...
  static {
    __name2(this, "GenerateVideosResponse");
  }
};
...
__name2(videoMetadataToMldev$4, "videoMetadataToMldev$4");
...
__name2(schemaToMldev$1, "schemaToMldev$1");
...
__name2(functionDeclarationToMldev$4, "functionDeclarationToMldev$4");
...
__name2(intervalToMldev$4, "intervalToMldev$4");
...
__name2(dynamicRetrievalConfigToMldev$4, "dynamicRetrievalConfigToMldev$4");
...
__name2(googleSearchRetrievalToMldev$4, "googleSearchRetrievalToMldev$4");
function urlContextToMldev$4() {
  const toObject = {};
  return toObject;
}
__name(urlContextToMldev$4, "urlContextToMldev$4");
__name2(urlContextToMldev$4, "urlContextToMldev$4");
...
__name2(latLngToMldev$2, "latLngToMldev$2");
...
__name2(retrievalConfigToMldev$2, "retrievalConfigToMldev$2");
...
__name2(prebuiltVoiceConfigToMldev$3, "prebuiltVoiceConfigToMldev$3");
...
__name2(voiceConfigToMldev$3, "voiceConfigToMldev$3");
...
__name2(speakerVoiceConfigToMldev$3, "speakerVoiceConfigToMldev$3");
...
__name2(multiSpeakerVoiceConfigToMldev$3, "multiSpeakerVoiceConfigToMldev$3");
...
__name2(speechConfigToMldev$3, "speechConfigToMldev$3");
...
__name2(thinkingConfigToMldev$1, "thinkingConfigToMldev$1");
...
__name2(videoMetadataFromMldev$2, "videoMetadataFromMldev$2");
...
__name2(blobFromMldev$2, "blobFromMldev$2");
...
__name2(fileDataFromMldev$2, "fileDataFromMldev$2");
...
__name2(partFromMldev$2, "partFromMldev$2");
...
__name2(contentFromMldev$2, "contentFromMldev$2");
...
__name2(urlMetadataFromMldev$2, "urlMetadataFromMldev$2");
...
__name2(urlContextMetadataFromMldev$2, "urlContextMetadataFromMldev$2");
...
__name2(jobErrorFromMldev, "jobErrorFromMldev");
...
__name2(jobErrorFromVertex, "jobErrorFromVertex");
...
__name2(videoMetadataToMldev$3, "videoMetadataToMldev$3");
...
__name2(functionDeclarationToMldev$3, "functionDeclarationToMldev$3");
...
__name2(intervalToMldev$3, "intervalToMldev$3");
...
__name2(dynamicRetrievalConfigToMldev$3, "dynamicRetrievalConfigToMldev$3");
...
__name2(googleSearchRetrievalToMldev$3, "googleSearchRetrievalToMldev$3");
function urlContextToMldev$3() {
  const toObject = {};
  return toObject;
}
__name(urlContextToMldev$3, "urlContextToMldev$3");
__name2(urlContextToMldev$3, "urlContextToMldev$3");
...
__name2(latLngToMldev$1, "latLngToMldev$1");
...
__name2(retrievalConfigToMldev$1, "retrievalConfigToMldev$1");
...
__name2(videoMetadataToVertex$2, "videoMetadataToVertex$2");
...
__name2(blobToVertex$2, "blobToVertex$2");
...
__name2(fileDataToVertex$2, "fileDataToVertex$2");
...
__name2(intervalToVertex$2, "intervalToVertex$2");
...
__name2(googleSearchToVertex$2, "googleSearchToVertex$2");
...
__name2(dynamicRetrievalConfigToVertex$2, "dynamicRetrievalConfigToVertex$2");
...
__name2(googleSearchRetrievalToVertex$2, "googleSearchRetrievalToVertex$2");
function enterpriseWebSearchToVertex$2() {
  const toObject = {};
  return toObject;
}
__name(enterpriseWebSearchToVertex$2, "enterpriseWebSearchToVertex$2");
__name2(enterpriseWebSearchToVertex$2, "enterpriseWebSearchToVertex$2");
...
__name2(apiKeyConfigToVertex$2, "apiKeyConfigToVertex$2");
...
__name2(authConfigToVertex$2, "authConfigToVertex$2");
...
__name2(googleMapsToVertex$2, "googleMapsToVertex$2");
function urlContextToVertex$2() {
  const toObject = {};
  return toObject;
}
__name(urlContextToVertex$2, "urlContextToVertex$2");
__name2(urlContextToVertex$2, "urlContextToVertex$2");
...
__name2(functionCallingConfigToVertex$1, "functionCallingConfigToVertex$1");
...
__name2(latLngToVertex$1, "latLngToVertex$1");
...
__name2(retrievalConfigToVertex$1, "retrievalConfigToVertex$1");
...
__name2(cachedContentFromMldev, "cachedContentFromMldev");
...
__name2(cachedContentFromVertex, "cachedContentFromVertex");
...
__name2(fileStatusToMldev, "fileStatusToMldev");
...
__name2(fileToMldev, "fileToMldev");
...
__name2(fileStatusFromMldev, "fileStatusFromMldev");
...
__name2(fileFromMldev, "fileFromMldev");
...
__name2(prebuiltVoiceConfigToMldev$2, "prebuiltVoiceConfigToMldev$2");
...
__name2(voiceConfigToMldev$2, "voiceConfigToMldev$2");
...
__name2(speakerVoiceConfigToMldev$2, "speakerVoiceConfigToMldev$2");
...
__name2(multiSpeakerVoiceConfigToMldev$2, "multiSpeakerVoiceConfigToMldev$2");
...
__name2(speechConfigToMldev$2, "speechConfigToMldev$2");
...
__name2(videoMetadataToMldev$2, "videoMetadataToMldev$2");
...
__name2(functionDeclarationToMldev$2, "functionDeclarationToMldev$2");
...
__name2(intervalToMldev$2, "intervalToMldev$2");
...
__name2(dynamicRetrievalConfigToMldev$2, "dynamicRetrievalConfigToMldev$2");
...
__name2(googleSearchRetrievalToMldev$2, "googleSearchRetrievalToMldev$2");
function urlContextToMldev$2() {
  const toObject = {};
  return toObject;
}
__name(urlContextToMldev$2, "urlContextToMldev$2");
__name2(urlContextToMldev$2, "urlContextToMldev$2");
...
__name2(automaticActivityDetectionToMldev$1, "automaticActivityDetectionToMldev$1");
...
__name2(realtimeInputConfigToMldev$1, "realtimeInputConfigToMldev$1");
...
__name2(slidingWindowToMldev$1, "slidingWindowToMldev$1");
...
__name2(contextWindowCompressionConfigToMldev$1, "contextWindowCompressionConfigToMldev$1");
...
__name2(proactivityConfigToMldev$1, "proactivityConfigToMldev$1");
...
__name2(activityStartToMldev, "activityStartToMldev");
function activityEndToMldev() {
  const toObject = {};
  return toObject;
}
__name(activityEndToMldev, "activityEndToMldev");
__name2(activityEndToMldev, "activityEndToMldev");
...
__name2(weightedPromptToMldev, "weightedPromptToMldev");
...
__name2(liveMusicGenerationConfigToMldev, "liveMusicGenerationConfigToMldev");
...
__name2(liveMusicClientSetupToMldev, "liveMusicClientSetupToMldev");
...
__name2(liveMusicClientContentToMldev, "liveMusicClientContentToMldev");
...
__name2(liveMusicClientMessageToMldev, "liveMusicClientMessageToMldev");
...
__name2(prebuiltVoiceConfigToVertex$1, "prebuiltVoiceConfigToVertex$1");
...
__name2(videoMetadataToVertex$1, "videoMetadataToVertex$1");
...
__name2(blobToVertex$1, "blobToVertex$1");
...
__name2(fileDataToVertex$1, "fileDataToVertex$1");
...
__name2(intervalToVertex$1, "intervalToVertex$1");
...
__name2(googleSearchToVertex$1, "googleSearchToVertex$1");
...
__name2(dynamicRetrievalConfigToVertex$1, "dynamicRetrievalConfigToVertex$1");
...
__name2(googleSearchRetrievalToVertex$1, "googleSearchRetrievalToVertex$1");
function enterpriseWebSearchToVertex$1() {
  const toObject = {};
  return toObject;
}
__name(enterpriseWebSearchToVertex$1, "enterpriseWebSearchToVertex$1");
__name2(enterpriseWebSearchToVertex$1, "enterpriseWebSearchToVertex$1");
...
__name2(apiKeyConfigToVertex$1, "apiKeyConfigToVertex$1");
...
__name2(authConfigToVertex$1, "authConfigToVertex$1");
...
__name2(googleMapsToVertex$1, "googleMapsToVertex$1");
function urlContextToVertex$1() {
  const toObject = {};
  return toObject;
}
__name(urlContextToVertex$1, "urlContextToVertex$1");
__name2(urlContextToVertex$1, "urlContextToVertex$1");
...
__name2(sessionResumptionConfigToVertex, "sessionResumptionConfigToVertex");
function audioTranscriptionConfigToVertex() {
  const toObject = {};
  return toObject;
}
__name(audioTranscriptionConfigToVertex, "audioTranscriptionConfigToVertex");
__name2(audioTranscriptionConfigToVertex, "audioTranscriptionConfigToVertex");
...
__name2(automaticActivityDetectionToVertex, "automaticActivityDetectionToVertex");
...
__name2(realtimeInputConfigToVertex, "realtimeInputConfigToVertex");
...
__name2(slidingWindowToVertex, "slidingWindowToVertex");
...
__name2(contextWindowCompressionConfigToVertex, "contextWindowCompressionConfigToVertex");
...
__name2(proactivityConfigToVertex, "proactivityConfigToVertex");
...
__name2(activityStartToVertex, "activityStartToVertex");
function activityEndToVertex() {
  const toObject = {};
  return toObject;
}
__name(activityEndToVertex, "activityEndToVertex");
__name2(activityEndToVertex, "activityEndToVertex");
...
__name2(liveServerSetupCompleteFromMldev, "liveServerSetupCompleteFromMldev");
...
__name2(videoMetadataFromMldev$1, "videoMetadataFromMldev$1");
...
__name2(blobFromMldev$1, "blobFromMldev$1");
...
__name2(fileDataFromMldev$1, "fileDataFromMldev$1");
...
__name2(partFromMldev$1, "partFromMldev$1");
...
__name2(contentFromMldev$1, "contentFromMldev$1");
...
__name2(transcriptionFromMldev, "transcriptionFromMldev");
...
__name2(urlMetadataFromMldev$1, "urlMetadataFromMldev$1");
...
__name2(urlContextMetadataFromMldev$1, "urlContextMetadataFromMldev$1");
...
__name2(liveServerContentFromMldev, "liveServerContentFromMldev");
...
__name2(functionCallFromMldev, "functionCallFromMldev");
...
__name2(liveServerToolCallFromMldev, "liveServerToolCallFromMldev");
...
__name2(liveServerToolCallCancellationFromMldev, "liveServerToolCallCancellationFromMldev");
...
__name2(modalityTokenCountFromMldev, "modalityTokenCountFromMldev");
...
__name2(usageMetadataFromMldev, "usageMetadataFromMldev");
...
__name2(liveServerGoAwayFromMldev, "liveServerGoAwayFromMldev");
...
__name2(liveServerSessionResumptionUpdateFromMldev, "liveServerSessionResumptionUpdateFromMldev");
...
__name2(liveServerMessageFromMldev, "liveServerMessageFromMldev");
...
__name2(liveMusicServerSetupCompleteFromMldev, "liveMusicServerSetupCompleteFromMldev");
...
__name2(weightedPromptFromMldev, "weightedPromptFromMldev");
...
__name2(liveMusicClientContentFromMldev, "liveMusicClientContentFromMldev");
...
__name2(liveMusicGenerationConfigFromMldev, "liveMusicGenerationConfigFromMldev");
...
__name2(liveMusicSourceMetadataFromMldev, "liveMusicSourceMetadataFromMldev");
...
__name2(audioChunkFromMldev, "audioChunkFromMldev");
...
__name2(liveMusicServerContentFromMldev, "liveMusicServerContentFromMldev");
...
__name2(liveMusicFilteredPromptFromMldev, "liveMusicFilteredPromptFromMldev");
...
__name2(liveMusicServerMessageFromMldev, "liveMusicServerMessageFromMldev");
...
__name2(liveServerSetupCompleteFromVertex, "liveServerSetupCompleteFromVertex");
...
__name2(videoMetadataFromVertex$1, "videoMetadataFromVertex$1");
...
__name2(blobFromVertex$1, "blobFromVertex$1");
...
__name2(fileDataFromVertex$1, "fileDataFromVertex$1");
...
__name2(partFromVertex$1, "partFromVertex$1");
...
__name2(contentFromVertex$1, "contentFromVertex$1");
...
__name2(transcriptionFromVertex, "transcriptionFromVertex");
...
__name2(liveServerContentFromVertex, "liveServerContentFromVertex");
...
__name2(functionCallFromVertex, "functionCallFromVertex");
...
__name2(liveServerToolCallFromVertex, "liveServerToolCallFromVertex");
...
__name2(liveServerToolCallCancellationFromVertex, "liveServerToolCallCancellationFromVertex");
...
__name2(modalityTokenCountFromVertex, "modalityTokenCountFromVertex");
...
__name2(liveServerGoAwayFromVertex, "liveServerGoAwayFromVertex");
...
__name2(liveServerSessionResumptionUpdateFromVertex, "liveServerSessionResumptionUpdateFromVertex");
...
__name2(videoMetadataToMldev$1, "videoMetadataToMldev$1");
...
__name2(schemaToMldev, "schemaToMldev");
...
__name2(functionDeclarationToMldev$1, "functionDeclarationToMldev$1");
...
__name2(intervalToMldev$1, "intervalToMldev$1");
...
__name2(dynamicRetrievalConfigToMldev$1, "dynamicRetrievalConfigToMldev$1");
...
__name2(googleSearchRetrievalToMldev$1, "googleSearchRetrievalToMldev$1");
function urlContextToMldev$1() {
  const toObject = {};
  return toObject;
}
__name(urlContextToMldev$1, "urlContextToMldev$1");
__name2(urlContextToMldev$1, "urlContextToMldev$1");
...
__name2(latLngToMldev, "latLngToMldev");
...
__name2(retrievalConfigToMldev, "retrievalConfigToMldev");
...
__name2(prebuiltVoiceConfigToMldev$1, "prebuiltVoiceConfigToMldev$1");
...
__name2(voiceConfigToMldev$1, "voiceConfigToMldev$1");
...
__name2(speakerVoiceConfigToMldev$1, "speakerVoiceConfigToMldev$1");
...
__name2(multiSpeakerVoiceConfigToMldev$1, "multiSpeakerVoiceConfigToMldev$1");
...
__name2(speechConfigToMldev$1, "speechConfigToMldev$1");
...
__name2(thinkingConfigToMldev, "thinkingConfigToMldev");
...
__name2(videoMetadataToVertex, "videoMetadataToVertex");
...
__name2(blobToVertex, "blobToVertex");
...
__name2(fileDataToVertex, "fileDataToVertex");
...
__name2(schemaToVertex, "schemaToVertex");
...
__name2(modelSelectionConfigToVertex, "modelSelectionConfigToVertex");
...
__name2(safetySettingToVertex, "safetySettingToVertex");
...
__name2(intervalToVertex, "intervalToVertex");
...
__name2(googleSearchToVertex, "googleSearchToVertex");
...
__name2(dynamicRetrievalConfigToVertex, "dynamicRetrievalConfigToVertex");
...
__name2(googleSearchRetrievalToVertex, "googleSearchRetrievalToVertex");
function enterpriseWebSearchToVertex() {
  const toObject = {};
  return toObject;
}
__name(enterpriseWebSearchToVertex, "enterpriseWebSearchToVertex");
__name2(enterpriseWebSearchToVertex, "enterpriseWebSearchToVertex");
...
__name2(apiKeyConfigToVertex, "apiKeyConfigToVertex");
...
__name2(authConfigToVertex, "authConfigToVertex");
...
__name2(googleMapsToVertex, "googleMapsToVertex");
function urlContextToVertex() {
  const toObject = {};
  return toObject;
}
__name(urlContextToVertex, "urlContextToVertex");
__name2(urlContextToVertex, "urlContextToVertex");
...
__name2(functionCallingConfigToVertex, "functionCallingConfigToVertex");
...
__name2(latLngToVertex, "latLngToVertex");
...
__name2(retrievalConfigToVertex, "retrievalConfigToVertex");
...
__name2(prebuiltVoiceConfigToVertex, "prebuiltVoiceConfigToVertex");
...
__name2(thinkingConfigToVertex, "thinkingConfigToVertex");
...
__name2(styleReferenceConfigToVertex, "styleReferenceConfigToVertex");
...
__name2(subjectReferenceConfigToVertex, "subjectReferenceConfigToVertex");
...
__name2(videoMetadataFromMldev, "videoMetadataFromMldev");
...
__name2(blobFromMldev, "blobFromMldev");
...
__name2(fileDataFromMldev, "fileDataFromMldev");
...
__name2(partFromMldev, "partFromMldev");
...
__name2(contentFromMldev, "contentFromMldev");
...
__name2(urlMetadataFromMldev, "urlMetadataFromMldev");
...
__name2(urlContextMetadataFromMldev, "urlContextMetadataFromMldev");
...
__name2(contentEmbeddingFromMldev, "contentEmbeddingFromMldev");
function embedContentMetadataFromMldev() {
  const toObject = {};
  return toObject;
}
__name(embedContentMetadataFromMldev, "embedContentMetadataFromMldev");
__name2(embedContentMetadataFromMldev, "embedContentMetadataFromMldev");
...
__name2(videoMetadataFromVertex, "videoMetadataFromVertex");
...
__name2(blobFromVertex, "blobFromVertex");
...
__name2(fileDataFromVertex, "fileDataFromVertex");
...
__name2(partFromVertex, "partFromVertex");
...
__name2(contentFromVertex, "contentFromVertex");
...
__name2(citationMetadataFromVertex, "citationMetadataFromVertex");
...
__name2(urlMetadataFromVertex, "urlMetadataFromVertex");
...
__name2(urlContextMetadataFromVertex, "urlContextMetadataFromVertex");
...
__name2(candidateFromVertex, "candidateFromVertex");
...
__name2(embedContentMetadataFromVertex, "embedContentMetadataFromVertex");
...
__name2(checkpointFromVertex, "checkpointFromVertex");
...
__name2(prebuiltVoiceConfigToMldev, "prebuiltVoiceConfigToMldev");
...
__name2(voiceConfigToMldev, "voiceConfigToMldev");
...
__name2(speakerVoiceConfigToMldev, "speakerVoiceConfigToMldev");
...
__name2(multiSpeakerVoiceConfigToMldev, "multiSpeakerVoiceConfigToMldev");
...
__name2(speechConfigToMldev, "speechConfigToMldev");
...
__name2(videoMetadataToMldev, "videoMetadataToMldev");
...
__name2(functionDeclarationToMldev, "functionDeclarationToMldev");
...
__name2(intervalToMldev, "intervalToMldev");
...
__name2(dynamicRetrievalConfigToMldev, "dynamicRetrievalConfigToMldev");
...
__name2(googleSearchRetrievalToMldev, "googleSearchRetrievalToMldev");
function urlContextToMldev() {
  const toObject = {};
  return toObject;
}
__name(urlContextToMldev, "urlContextToMldev");
__name2(urlContextToMldev, "urlContextToMldev");
...
__name2(automaticActivityDetectionToMldev, "automaticActivityDetectionToMldev");
...
__name2(realtimeInputConfigToMldev, "realtimeInputConfigToMldev");
...
__name2(slidingWindowToMldev, "slidingWindowToMldev");
...
__name2(contextWindowCompressionConfigToMldev, "contextWindowCompressionConfigToMldev");
...
__name2(proactivityConfigToMldev, "proactivityConfigToMldev");
...
__name2(authTokenFromMldev, "authTokenFromMldev");
...
__name2(tuningExampleToMldev, "tuningExampleToMldev");
...
__name2(createTuningJobParametersToMldev, "createTuningJobParametersToMldev");
...
__name2(createTuningJobParametersToVertex, "createTuningJobParametersToVertex");
...
__name2(tunedModelCheckpointFromVertex, "tunedModelCheckpointFromVertex");
...
__name2(tunedModelFromVertex, "tunedModelFromVertex");
...
var metaPromptTranslations = {
  ...
  fr: {
    systemInstructionBase: "Vous \xEAtes un ing\xE9nieur de prompts expert. G\xE9n\xE9rez un prompt complet et ex\xE9cutable suivant exactement le format : <System>, <User>, <Example>. Le prompt final doit \xEAtre en {TARGET_LANGUAGE} et pr\xEAt \xE0 \xEAtre utilis\xE9 imm\xE9diatement par une IA. Incluez une m\xE9thodologie d\xE9taill\xE9e dans la section User et des exemples convaincants. IMPORTANT: Commencez votre r\xE9ponse par un titre concis (5-8 mots maximum) en utilisant ce format EXACT :\n\nTITRE: [Votre titre concis ici]\n\nPROMPT:\n[Votre prompt complet ici]\n\nNe g\xE9n\xE9rez QUE le titre et le prompt - aucun m\xE9ta-commentaire ou explication en dehors de cette structure.",
    ...
    mvpTitleInstruction: "Cr\xE9ez un titre concis et professionnel (5-8 mots maximum) qui r\xE9sume l'objectif du prompt. Concentrez-vous sur l'action principale et le sujet. Exemples : 'Conception Cours Strat\xE9gie Marketing', 'Analyse Retours Clients', 'Cr\xE9ation Module Formation'.",
    mvpFooter: "CRITIQUE: Fournissez d'abord le titre en utilisant le format 'TITRE: [votre titre]', puis g\xE9n\xE9rez le prompt complet avec les sections <System>, <User>, et <Example>. N'ajoutez aucun m\xE9ta-commentaire ou explication en dehors de cette structure.",
    ...
    agenticTitleInstruction: "Cr\xE9ez un titre concis et professionnel (5-8 mots maximum) qui capture l'objectif du prompt agentique. Concentrez-vous sur le but analytique ou cr\xE9atif principal. Exemples : 'Outil Analyse Marketing Strat\xE9gique', 'Constructeur Module Apprentissage', 'Assistant Cr\xE9ation Contenu'.",
    agenticFooter: "CRITIQUE: Fournissez d'abord le titre en utilisant le format 'TITRE: [votre titre]', puis g\xE9n\xE9rez le prompt complet avec les sections <System>, <User>, et <Example>. N'ajoutez aucun m\xE9ta-commentaire ou explication en dehors de cette structure."
  }
};
...
__name(generateUUID2, "generateUUID2");
__name2(generateUUID2, "generateUUID");
function generatePromptTitle(params, language) {
  const { rawRequest, domain, promptType, expertRole, mission } = params;
  const domainKeywords = {
    fr: {
      education: ["Cours", "Formation", "Apprentissage", "P\xE9dagogie", "Enseignement"],
      technical: ["Analyse", "D\xE9veloppement", "Syst\xE8me", "Solution", "Architecture"],
      creative: ["Cr\xE9ation", "Design", "R\xE9daction", "Conception", "Innovation"],
      analysis: ["\xC9tude", "Analyse", "\xC9valuation", "Rapport", "Recherche"],
      other: ["Projet", "T\xE2che", "Mission", "Activit\xE9", "D\xE9veloppement"]
    },
    en: {
  ...
  const actionPatterns = {
    fr: {
      "cr\xE9er|g\xE9n\xE9r|d\xE9velopp|concevoir|\xE9labor": "Cr\xE9ation",
      "transform|convert|adapt|modifi": "Transformation",
      "analys|\xE9valu|\xE9tudi|examin": "Analyse",
      "enseign|form|apprend|expliqu": "Formation",
      "organis|planifi|structur": "Organisation",
      "r\xE9soudr|r\xE9pond|aider|assist": "Assistance"
    },
    en: {
...
__name(generatePromptTitle, "generatePromptTitle");
__name2(generatePromptTitle, "generatePromptTitle");
function parseAIResponse(response, language) {
  try {
    const titlePattern = language === "fr" ? /^TITRE:\s*(.+?)(?:\n|$)/im : /^TITLE:\s*(.+?)(?:\n|$)/im;
    const fallbackTitlePattern = language === "fr" ? /^TITLE:\s*(.+?)(?:\n|$)/im : /^TITRE:\s*(.+?)(?:\n|$)/im;
...
__name2(parseAIResponse, "parseAIResponse");
...
__name2(buildPromptQuery, "buildPromptQuery");
...
var onRequestGet3 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    let user;
    try {
      user = await verifyJWT3(token, env.JWT_SECRET);
    } catch (error) {
      const errorResponse = new Response(JSON.stringify({
        success: false,
...
@google/genai/dist/web/index.mjs:
   ...
   *)
*/
//# sourceMappingURL=functionsWorker-0.1729981883470495.js.map
